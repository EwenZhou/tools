...
class Trains_Demo():
	...
	def __init__(self, raw_trains, option):
		# self.headers = '车次 车站 始终车站 时间 历时 商务/特等座 一等座 二等座 高级软卧 软卧 动卧 硬卧 软座 硬座 无座 其他'.split()
		self.headers = '车次 车站 始终车站 时间 历时 商务/特等座 一等座 二等座 高级软卧 软卧 动卧 硬卧 软座 硬座 无座'.split()
	...
	# 获得起点和终点站
	def get_all_from_to_station_name(self,data_list):
		self.all_from_station_name = data_list[4]
		self.all_to_station_name = data_list[5]
		self.all_from_to_station_name = Station_Parse.parse_station().disparse(self.all_from_station_name) + '-->' + Station_Parse.parse_station().disparse(self.all_to_station_name)
		return self.all_from_to_station_name
	# 获得出发和到达时间
	...
	def parse_trains_data(self, data_list):
		return {
			...
			'all_from_to_station_name':self.get_all_from_to_station_name(data_list),
	...
	def show_trian_data(self):
		...
		for self.train in self.raw_trains:
			...
			if self.need_show(self.data_list):
				...
				self.values_row.append(self.parsed_train_data['all_from_to_station_name'])
...
class Buy_Tickets(object):
	...
	def __init__(self, username, passwd, order, passengers, dtime, starts, ends):
		...
		self.executable_path = 'D:\\Program Files\\Python\\Python37\\Scripts\\chromedriver.exe'
	# 登录
	def login(self):
	...
	def __init__(self):
		...
		while True:
			...
			if self.select == 'query':
				...
				try:
					print(self.date)
					if datetime.strptime(self.date, '%Y-%m-%d') < datetime.now()-timedelta(days=1):
						raise ValueError
				except:
					print('请输入有效日期...')
					continue
			...
			# 抢票功能
				...
				print('请逐个输入需要购票的乘客名，输入完成后请输入end.')
				print('普通票输入例如：张三，学生票输入例如：张三(学生)')
				self.passengers = []
				while True:
					self.passenger = input('name or end：')
					if self.passenger == 'end':
						break
					else:
				...
				try:
					...
					continue
				try:
					if datetime.strptime(self.dtime, '%Y-%m-%d') < datetime.now()-timedelta(days=1):
						raise ValueError
				except:
					print('请输入有效日期...')
					continue
